...
void CDirStatDoc::DeleteContents()
{
    ...
    RemoveMapItemDetails(m_rootItem);
...
BOOL CDirStatDoc::OnOpenDocument(CItem* newroot)
{
    ...
    // Populate the Largest Files list from the pre-built tree (after UpdateAllViews
    // which resets the top control via SetRootItem(nullptr))
    CFileTopControl::Get()->SetRootItem(nullptr);
...
COLORREF CDirStatDoc::GetCushionColor(const std::wstring & ext)
{
    ...
    // Handle empty extension string
    if (ext.empty())
    {
        return RGB(128, 128, 128);  // Default gray for unknown extensions
    }

    const auto record = m_extensionData.find(ext);
    if (record == m_extensionData.end())
    {
        // Extension data may still be rebuilding or the extension may be unknown.
        // Use a stable fallback instead of breaking rendering with an assertion.
        return RGB(128, 128, 128);  // Default gray for missing extensions
    }
...
{
    // This method creates an entry if it doesn't exist
    // The caller should not hold this pointer across lock releases
...
void CDirStatDoc::RebuildExtensionData()
{
    ...
    // If no extensions found, nothing to do
    if (sortedExtensions.empty())
    {
        return;
    }
    ...
    // Guard against empty color palette
    if (colors.empty())
    {
        return;
    }
...
{
    // Request the file path from the user
    const std::wstring fileSelectString = std::format(L"{} (*.csv;*.json;*.map)|*.csv;*.json;*.map|{} (*.*)|*.*||",
        Localization::Lookup(IDS_FILE_FILTER), Localization::Lookup(IDS_ALL_FILES));
    ...
    if (path.size() >= 4 && _wcsicmp(path.c_str() + path.size() - 4, L".map") == 0)
    {
        ...
        if (!regions.empty())
        {
            ...
            if (regionDlg.DoModal() != IDOK) return;
