...
namespace
{
bool HasExtensionInsensitive(const std::wstring_view path, const std::wstring_view extension)
{
    return path.size() >= extension.size() &&
        _wcsicmp(path.data() + path.size() - extension.size(), extension.data()) == 0;
}

bool IsImportedResultsPath(const std::wstring_view path)
{
    return HasExtensionInsensitive(path, L".csv") || HasExtensionInsensitive(path, L".json");
}

bool IsMapLikePath(const std::wstring_view path)
{
    return HasExtensionInsensitive(path, L".map") || HasExtensionInsensitive(path, L".elf");
}

CItem* LoadImportedTree(const std::wstring& path)
{
    if (HasExtensionInsensitive(path, L".map"))
    {
        const auto regions = GetMapRegions(path);
    ...
    {
        auto mapPath = std::filesystem::path(path);
        mapPath.replace_extension(L".map");
        if (!std::filesystem::exists(mapPath))
        {
            return nullptr;
        }

        const auto regions = GetMapRegions(mapPath.wstring());
        std::optional<std::wstring> selectedRegion;
        if (!regions.empty())
        {
            CSelectRegionDlg regionDlg(regions, AfxGetMainWnd());
...
class CMemMapExplorerCommandLineInfo final : public CCommandLineInfo
{
...
BOOL CDirStatApp::InitInstance()
{
    ...
    CMemMapExplorerCommandLineInfo cmdInfo;
    ...
    {
        if (CItem* newroot = LoadImportedTree(m_loadFromPath); newroot != nullptr)
        {
    ...
    else if (const std::wstring path = cmdInfo.m_strFileName.GetString();
        IsImportedResultsPath(path) || IsMapLikePath(path))
    {
        if (CItem* newroot = LoadImportedTree(path); newroot != nullptr)
        {
            CDirStatDoc::Get()->OnOpenDocument(newroot);
        }
        else
        {
            DisplayError(L"Could not import the selected analysis file.");
        }
    }
    else
    {
